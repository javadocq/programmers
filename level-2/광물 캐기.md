...
def solution(picks, minerals):
    ...
    picks_sum = sum(picks) * 5
    minerals = minerals[:picks_sum]

    minerals_list = []

    for i in range(0, len(minerals), 5):
        temp_list = minerals[i:i+5]
        dia = temp_list.count("diamond")
        iron = temp_list.count("iron")
        stone = temp_list.count("stone")
        minerals_list.append([dia,iron,stone])

    minerals_list.sort(key=lambda x : (x[0], x[1], x[2]), reverse=True)

    for da in minerals_list:
        d, i, s = da
        if picks[0] > 0:
            picks[0] -= 1
            answer += (d + i + s)
