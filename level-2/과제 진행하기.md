...
def solution(plans):
    ...
    for i in range(len(plans)):
        ...
        new_start = int(time_start[0]) * 60 + int(time_start[1])
        end = new_start + int(playtime)
        time_change_plans.append([name, new_start, end, int(playtime)])

    time_change_plans.sort(key=lambda x : x[1])
    curr_time = 0

    for plan in time_change_plans:
        name, start_time, end_time, play_time = plan
        while st:
            p_name, p_remain = st.pop()

            if curr_time + p_remain <= start_time:
                curr_time += p_remain
                answer.append(p_name)
            else:
                p_remain = p_remain - (start_time - curr_time)
                st.append([p_name, p_remain])
                break

        curr_time = start_time
        st.append([name, (end_time - start_time)])
