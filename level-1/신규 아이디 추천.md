...
string solution(string new_id) {
    ...
    while(j < answer.length()) {
        if(!((answer[j] >= 'a' && answer[j] <= 'z') || (answer[j] >= '0' && answer[j] <= '9') || answer[j] == '-' || answer[j] == '_' || answer[j] == '.')) {
            answer.erase(j,1);
            continue;
        }
        j++;
    ...
    //step 3
    while(true) {
        size_t npos = answer.find("..");
        if(npos <= answer.length()) {
            answer.replace(npos,2,".");
        } else {
            break;
        }
    }
    ...
    //step 7
    char ch = answer2[answer2.length() -1];
    while(answer2.length() <= 2) {
